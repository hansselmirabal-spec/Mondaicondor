  ...
  return (
    ...
    <div className="flex items-center gap-1 px-4 py-2 bg-white border-b border-gray-200 flex-wrap">
      ...
      <button
        ...
        className="flex items-center gap-1.5 px-3 py-1.5 bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium rounded-md transition-colors mr-2"
      >
      ...
      <button
        ...
        className={`md:hidden flex items-center gap-1.5 px-2.5 py-1.5 text-sm rounded-md transition-colors ml-auto ${
          hasActiveFilters ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:bg-gray-100'
        }`}
      >
        <SlidersHorizontal className="w-3.5 h-3.5" />
        {hasActiveFilters && <span className="w-1.5 h-1.5 rounded-full bg-primary-600" />}
      </button>
      ...
      <Dropdown
        ...
        onClose={close}
        trigger={
          <button onClick={() => toggle('persona')} className={`flex items-center gap-1 px-2 py-1.5 text-sm rounded-md transition-colors ${selectedPersonaId ? 'bg-primary-50 text-primary-700 font-medium' : 'text-gray-600 hover:bg-gray-100'}`}>
      ...
      <Dropdown
        ...
        width="w-64"
        trigger={
          <button onClick={() => toggle('filter')} className={`flex items-center gap-1 px-2 py-1.5 text-sm rounded-md transition-colors ${(filterStatus || filterPriority) ? 'bg-primary-50 text-primary-700 font-medium' : 'text-gray-600 hover:bg-gray-100'}`}>
            ...
            <div className="flex flex-wrap gap-1">
              {workspaceStatuses.map(s => (
                <button
                  ...
                  className={`px-2 py-0.5 rounded text-xs font-medium border transition-colors ${filterStatus === s.slug ? 'bg-primary-600 text-white border-primary-600' : 'border-gray-200 text-gray-600 hover:border-primary-400'}`}
                >{s.label}</button>
              ))}
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-1">
              {PRIORITIES.map(p => (
                <button
                  ...
                  className={`px-2 py-0.5 rounded text-xs font-medium border transition-colors ${filterPriority === p ? 'bg-primary-600 text-white border-primary-600' : 'border-gray-200 text-gray-600 hover:border-primary-400'}`}
                >{p}</button>
              ))}
            </div>
          </div>
              ...
              <div className="flex flex-wrap gap-1">
                {workspaceUens.map(u => (
                  <button
                    ...
                    className={`px-2 py-0.5 rounded text-xs font-medium border transition-colors ${filterUenId === u.id ? 'text-white border-transparent' : 'border-gray-200 text-gray-600 hover:border-primary-400'}`}
      ...
      <Dropdown
        ...
        onClose={close}
        trigger={
          <button onClick={() => toggle('sort')} className={`flex items-center gap-1 px-2 py-1.5 text-sm rounded-md transition-colors ${sortField ? 'bg-primary-50 text-primary-700 font-medium' : 'text-gray-600 hover:bg-gray-100'}`}>
      ...
      <Dropdown
        ...
        onClose={close}
        trigger={
          <button onClick={() => toggle('hide')} className={`flex items-center gap-1 px-2 py-1.5 text-sm rounded-md transition-colors ${hiddenColumns.length > 0 ? 'bg-primary-50 text-primary-700 font-medium' : 'text-gray-600 hover:bg-gray-100'}`}>
        ...
        <div className="p-2 space-y-0.5">
          {pickerColumns.map(({ id, label }) => {
            ...
            return (
              ...
              >
                <span className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 ${hidden ? 'border-gray-200 bg-white' : 'border-primary-600 bg-primary-600'}`}>
      ...
      <Dropdown
        ...
        onClose={close}
        trigger={
          <button onClick={() => toggle('group')} className={`flex items-center gap-1 px-2 py-1.5 text-sm rounded-md transition-colors ${groupBy !== 'default' ? 'bg-primary-50 text-primary-700 font-medium' : 'text-gray-600 hover:bg-gray-100'}`}>
      ...
      <div className="p-5 space-y-4">
            ...
            {apiUsers.map(u => {
              ...
              return (
                ...
                >
                  ...
                  {u.name.split(' ')[0]}
                  {selected && <Check className="w-3 h-3 text-primary-500 ml-0.5" />}
                </button>
              )
            })}
          </div>
          {newTaskAssigneeIds.length > 0 && (
            <p className="text-xs text-primary-600 mt-1">{newTaskAssigneeIds.length} responsable{newTaskAssigneeIds.length > 1 ? 's' : ''} seleccionado{newTaskAssigneeIds.length > 1 ? 's' : ''}</p>
          )}
        </div>
        ...
        {workspaceUens.length > 0 && (
          <div>
              ...
              {workspaceUens.map(u => {
                ...
                return (
                  <button
                    ...
                    className={`px-2 py-0.5 rounded text-xs font-medium border transition-colors ${selected ? 'text-white border-transparent' : 'border-gray-200 text-gray-600 hover:border-primary-400'}`}
            ...
            {newTaskUenIds.length > 0 && (
              <p className="text-xs text-primary-600 mt-1">{newTaskUenIds.length} UEN{newTaskUenIds.length > 1 ? 's' : ''} seleccionada{newTaskUenIds.length > 1 ? 's' : ''}</p>
            )}
          </div>
        )}
        ...
        <div className="flex gap-2 pt-1">
          ...
          <button
            ...
            className="flex-1 py-2 text-sm bg-primary-600 hover:bg-primary-700 disabled:bg-primary-200 text-white font-medium rounded-lg transition-colors"
          >
